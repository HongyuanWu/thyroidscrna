...
xii<-read.table("https://raw.githubusercontent.com/Shicheng-Guo/thyroidscrna/master/BRAFV600E/list.txt",sep="\t",as.is=T)[,1]
...
setwd("/mnt/bigdata/Genetic/Projects/shg047/tcga/")
setwd("/mnt/bigdata/Genetic/Projects/shg047/tcga/brafv600e/meta")
...
for(i in ii){
  ...
  if(! is.na(fixedEffect[4]) & fixedEffect[4]<0.001){
...
rownames(out2)<-ENSG2Symbol(rownames(input)[ii])

write.csv(out2,file=paste("BRAFV600E","OS.HR.csv",sep=""),quote=F)
write.table(out2,file=paste("BRAFV600E","OS.HR.txt",sep=""),quote=F,sep="\t",col.names=NA,row.names=T)
